...
async function main() {
    ...
    console.log(`User provided ${userIds.length} unique IDs (after cleaning).`);
    ...
    const userIdsInDb = userIds.filter(id => dbIds.has(id));
    const userIdsNotInDb = userIds.filter(id => !dbIds.has(id));

    console.log(`IDs in Database: ${userIdsInDb.length}`);
    console.log(`IDs NOT in Database: ${userIdsNotInDb.length} (already deleted or wrong ID)`);

    let heuristicTestCount = 0;
    const heuristicOnlyIds = [];
    const bothIds = [];
    const userOnlyIds = [];
    ...
    for (const s of submissions) {
        ...
        if (result.isTest && isInUserList) {
            ...
            heuristicOnlyIds.push(s.id);
        } else if (isInUserList) {
            userOnlyIds.push(s.id);
        }
    }

    console.log(`\nComparison Details (Current DB Only):`);
    console.log(`Match Both (Heuristic + User): ${bothIds.length}`);
    console.log(`Heuristic ONLY (Flagged by AI but not user): ${heuristicOnlyIds.length}`);
    console.log(`User ONLY (Verified by user but not AI): ${userOnlyIds.length}`);

    const totalToDelete = new Set([...bothIds, ...userOnlyIds, ...heuristicOnlyIds]);
    console.log(`\nTotal Unique Test Submissions to delete: ${totalToDelete.size}`);
